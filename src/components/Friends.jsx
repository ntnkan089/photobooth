...
const FriendsRec = () => {
  ...
  const handleAddFriend = async (userId) => {
    try {
      ...
      if (response.ok) {
        ...
        setAddedFriends([...addedFriends, userId]); // Update the list of added friends
      } else {
  ...
  const isFriendAdded = (userId) => {
    return addedFriends.includes(userId);
  };
    ...
    <div className='friends-rec'>
      ...
      {recommendations.map(friend => (
        <div key={friend.user_id} className='friend-rec-item'>
          ...
          <button 
            className={`add-friend-button ${isFriendAdded(friend.user_id) ? 'added' : ''}`}
            onClick={() => handleAddFriend(friend.user_id)}
            disabled={isFriendAdded(friend.user_id)}
          >
            {isFriendAdded(friend.user_id) ? 'Added' : 'Add Friend'}
          </button>
        </div>
      ))}
    </div>
